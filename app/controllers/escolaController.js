...
const escolaController = {
    ...
    mostrarPerfilEscola: async (req, res) => {
        try {
            let results = await escolaModel.findId(req.session.autenticado.id);
            let campos = {
                name_school: results[0].nome_escola,
                adress: results[0].endereco,
                ...
                tipos_ensino: results[0].tipos_ensino ? results[0].tipos_ensino.split(',') : [],
                turnos: results[0].turnos ? results[0].turnos.split(',') : [],
                redes: results[0].redes ? results[0].redes.split(',') : []
            };
            res.render("pages/editar-escola", { erros: null, dadosNotificacao: null, valores: campos });
        } catch (e) {
            console.log(e);
            res.render("pages/editar-escola", {
                erros: null, dadosNotificacao: null, valores: {
                    name_school: "", adress: "", adress_n: "", city: "", state: "", email: "", cnpj: "", cep: "",
                    img_perfil_banco: "", img_perfil_pasta: "", tipos_ensino: [], turnos: [], redes: []
                }
            });
        }
    },
